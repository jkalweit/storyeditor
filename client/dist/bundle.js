...
var __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__, exports, __webpack_require__(52)], __WEBPACK_AMD_DEFINE_RESULT__ = function (require, exports, scanner_1) {
    ...
    function parseChapter() {
        ...
        while (!isTerminator(indentation)) {
            ...
            var promptIndentation = indentation + 1;
            while (token() === 5 /* IndentationToken */ && !isTerminator(indentation)) {
    ...
    function isTerminator(indentation) {
    ...
    var NodeKind;
    (function (NodeKind) {
        NodeKind[NodeKind["Chapter"] = 0] = "Chapter";
        NodeKind[NodeKind["Prompt"] = 1] = "Prompt";
        NodeKind[NodeKind["PromptOption"] = 2] = "PromptOption";
        NodeKind[NodeKind["Next"] = 3] = "Next";
    ...
    function parsePrompt(indentation) {
        var name = tokenValue();
        var text = '';
        var options = [];
        var next = undefined;
    ...
    function parseOption() {
        var indentation = countIndentation(tokenValue());
        nextToken();
        if (token() !== 6 /* StringLiteral */) {
            error('Expected option text.');
        }
        var text = tokenValue();
        nextToken();
        if (token() !== 19 /* FatArrowToken */) {
                    ...
                    case 61 /* equals */:
                        if (text.charCodeAt(pos + 1) === 62 /* greaterThan */) {
                            ...
                            while (pos < end && isWhiteSpace(text.charCodeAt(pos)))
                                pos++;
                            tokenValue = scanIdentifierParts();
                            return token = 19 /* FatArrowToken */;
                        }
                        return 20 /* EqualsToken */;
                    /*
                case CharacterCodes.backtick:
                    return token = scanTemplateAndSetTokenValue();
                    */
                    case 40 /* openParen */:
                        tokenValue = scanArguments();
...
!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__, exports, __webpack_require__(21), __webpack_require__(22), __webpack_require__(57)], __WEBPACK_AMD_DEFINE_RESULT__ = function (require, exports, __SyncNode_SyncView_1, parser_1, smoothscroll) {
    ...
    var MainView = (function (_super) {
        ...
        function MainView(options) {
            ...
            _this.storyList.on('selected', function (story) {
                ...
                _this.editor.update(story);
            });
            _this.addBinding('storyList', 'update', 'data.stories');
            return _this;
        }
        MainView.prototype.render = function () {
            ...
            if (this.selectedStory)
                this.selectedStory = this.data.stories[this.selectedStory.key];
            if (!this.first) {
                this.first = true;
                this.selectedStory = __SyncNode_SyncView_1.SyncUtils.toArray(this.data.stories)[0];
            }
    ...
    var StoryList = (function (_super) {
        ...
        function StoryList(options) {
            ...
            _this.addBtn.addEventListener('click', function () {
                ...
                _this.data.setItem(story);
            });
            _this.storyItemList.on('selected', function (view, story) { _this.emit('selected', story); });
            _this.addBinding('storyItemList', 'update', 'data');
            return _this;
        }
        return StoryList;
    }(__SyncNode_SyncView_1.SyncView));
    ...
    var StoryItem = (function (_super) {
        ...
        function StoryItem(options) {
            if (options === void 0) { options = {}; }
            var _this = _super.call(this, options) || this;
            _this.title = _this.add('span', { "innerHTML": "", "className": "" });
            _this.el.className += ' ';
            _this.el.className += ' StoryItem_style';
            _this.el.addEventListener('click', _this.onClick.bind(_this));
            _this.addBinding('title', 'innerHTML', 'data.title');
            return _this;
        }
        StoryItem.prototype.onClick = function () {
            this.emit('selected', this.data);
        };
        return StoryItem;
    }(__SyncNode_SyncView_1.SyncView));
    exports.StoryItem = StoryItem;
    var StoryEditorControls = (function (_super) {
        __extends(StoryEditorControls, _super);
        function StoryEditorControls(options) {
            if (options === void 0) { options = {}; }
            var _this = _super.call(this, options) || this;
        ...
        return StoryEditorControls;
    }(__SyncNode_SyncView_1.SyncView));
    exports.StoryEditorControls = StoryEditorControls;
    var StoryAndPlayer = (function (_super) {
        __extends(StoryAndPlayer, _super);
        ...
        StoryAndPlayer.prototype.parse = function () {
            try {
                var prog = parser_1.parse(this.data.text);
    ...
    __SyncNode_SyncView_1.SyncView.addGlobalStyle('.StoryEditorControls_controls_style', " padding-bottom: 1em; ");
    var Player = (function (_super) {
        ...
        Player.prototype.play = function (prog) {
            this.render();
            this.prog = prog;
            this.output.innerHTML = '';
            var firstChapter = prog[0];
            this.displayPrompt(firstChapter.prompts[0]);
        };
        Player.prototype.displayPrompt = function (prompt, args) {
            var _this = this;
            if (args === void 0) { args = ''; }
            ...
            var innerHTML = prompt.text;
            if (prompt.args && args) {
                var promptArgs = prompt.args.split(',');
                var givenArgs = args.split(',');
                for (var i = 0; i < promptArgs.length && i < givenArgs.length; i++) {
                    // Remove double quotes at beginning and end:
                    var val = givenArgs[i].replace(/^"(.+(?="$))"$/, '$1');
                    ;
            ...
            if (prompt.next) {
                ...
                prompt.options.forEach(function (option) {
                    _this.displayPromptOption(option, delay);
                    delay += 500;
                });
            }
        ...
        Player.prototype.displayPromptByName = function (name, args) {
            var _this = this;
            if (args === void 0) { args = ''; }
            name = name.trim();
            var found = false;
            this.prog.forEach(function (chapter) {
                chapter.prompts.forEach(function (prompt) {
                    if (prompt.name.toLowerCase() === name) {
                        _this.displayPrompt(prompt, args);
                        found = true;
                    }
                });
            });
            if (!found)
                console.error('Could not find prompt: ', name);
        };
        Player.prototype.displayNext = function (next) {
            var _this = this;
            var name = next.next.toLowerCase();
            if (name === 'wait') {
                var args = next.args.split(',');
                if (args.length >= 2) {
                    var waitTime = parseInt(args[0]);
                    if (waitTime === NaN) {
                        console.error('Invalid wait time');
                    }
                    var nextPromptName_1 = args[1];
                    if (this.waitDisabled)
                        waitTime = 0;
                    setTimeout(function () {
                        _this.displayPromptByName(nextPromptName_1);
                    }, waitTime * 1000);
                }
                else {
        ...
        Player.prototype.displayPromptOption = function (option, delay) {
            var _this = this;
            var out = document.createElement('div');
